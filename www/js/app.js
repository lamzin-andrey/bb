	...
	*/
	function initSigninButton() {
		...
		function _loginAction() {
			req({token: $('#token').val(), phone:$('#login').val(), password:$('#password').val()}, _onSuccess, defaultAjaxFail, 'login', WEB_ROOT + '/login');
			return false;
		}
		...
		$("#breg").click(
			function () {
				var pwd = $('#rpassword').val(), pwdC = $('#password_confirm').val(), email = $('#rlogin').val(),
					...
					phone = $('#rphone').val();
				if (pwd == pwdC && pwd.length && phone.length) {
					data = {email:email, phone:phone, password:pwd, pc:pwdC, name: name, sname: sname, token: $('#token').val()};
					if ($('#regfstr')[0]) {
						data.regfstr = $('#regfstr').val();
					}
					req(data, _onSuccess, defaultAjaxFail, 'signup', WEB_ROOT + '/login');
				} else {
					showError(lang['phone_required'] + ' ' + lang['and_password_required']);
				}
				//appWindowClose();
			}
		);
