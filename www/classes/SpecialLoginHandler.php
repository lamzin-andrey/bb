...
class SpecialLoginHandler extends LoginHandler {
	...
	**/
	protected function _signup($use_json = true) {
		...
		$exists = dbvalue("SELECT id FROM users WHERE phone = '{$phone}'");
		if ($exists) {
			//json_error('sError', $lang['email_already_exists']);
		...
		$sql_query = "UPDATE users SET name = '{$name}', surname = '{$surname}', email = '{$email}', pwd = '{$pwd}', phone = '{$phone}' WHERE id = {$uid}";
		//die($sql_query);
